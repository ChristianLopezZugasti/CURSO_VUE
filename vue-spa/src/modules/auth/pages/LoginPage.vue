...
const onLogin = () => {
    //es u espacio en el web, donde se puede almacenar infomracion , strings
    //aqui simulamos que estamos autenticados 
    localStorage.setItem('userID','ABC-123')  
    //si no existe, guardamos el / 
    const lastPath = localStorage.getItem('lasPath') ?? '/'
     
    //router.replace({
    //    name:'home',
    // })
    //Aqui te va  direccionar a la ultima pagina donde estuviste
